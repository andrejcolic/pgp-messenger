...
def _encrypt_3des(plaintext: bytes, key: bytes, iv: bytes) -> bytes:
    algorithm = TripleDES(key)
    mode = modes.CFB(iv)
    encryptor = Cipher(algorithm,mode=mode).encryptor()
    return encryptor.update(plaintext)

def _decrypt_3des(ciphertext: bytes, key: bytes, iv: bytes) -> bytes:
    algorithm = TripleDES(key)
    mode = modes.CFB(iv)
    decryptor = Cipher(algorithm, mode=mode).decryptor()
    return decryptor.update(ciphertext)

def _encrypt_aes128(plaintext: bytes, key: bytes, iv: bytes) -> bytes:
    algorithm = AES128(key)
    mode = modes.CFB(iv)
    encryptor = Cipher(algorithm,mode=mode).encryptor()
    return encryptor.update(plaintext)

def _decrypt_aes128(ciphertext: bytes, key: bytes, iv: bytes) -> bytes:
    algorithm = AES128(key)
    mode = modes.CFB(iv)
    decryptor = Cipher(algorithm,mode=mode).decryptor()
    return decryptor.update(ciphertext)
...
_ALGOS = {
    "TripleDES": { "encrypt": _encrypt_3des, "decrypt": _decrypt_3des , "iv_size": 8},
    "AES128": { "encrypt": _encrypt_aes128, "decrypt": _decrypt_aes128 , "iv_size": 16}
}

def generate_iv(algorithm: str) -> bytes:
    """
    Generates a random initialization vector (IV) for the specified symmetric algorithm.
    :param algorithm:
    :return:
    """
...
def encrypt(algorithm: str, plaintext: str | bytes, key: str | bytes, iv: bytes) -> bytes:
    """
    Encrypts the given plaintext using the specified symmetric algorithm, key, and initialization vector (IV).
    :param algorithm:
    :param plaintext:
    :param key:
    :param iv:
    ...
    if algorithm not in _ALGOS:
        raise ValueError(f"Nevalidan algoritam: {algorithm}")
...
def decrypt(algorithm: str, ciphertext: str | bytes, key: str | bytes, iv: bytes) -> bytes:
    """
    Decrypts the given ciphertext using the specified symmetric algorithm, key, and initialization vector (IV).
    :param algorithm:
    :param ciphertext:
    :param key:
    :param iv:
